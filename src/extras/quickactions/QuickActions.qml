// Portado de Serpantinum: src/quickshell/quickactions/Floating.qml (AGPL-3.0)
import QtQuick
...
Variants {
    ...
    delegate: Component {
        StyledWindow {
            ...
            WlrLayershell.keyboardFocus: floatingWidget.isSidebarVisible ? WlrKeyboardFocus.OnDemand : WlrKeyboardFocus.None
            ...
            // Dock is a separate window (lane C); the Quick Actions host owns the
            // full screen border. Edge B regions are kept at width/height 0 so the
            // mask/MouseArea structure mirrors the original Floating.qml.
            readonly property real topEdgeA_x: leftBound
            readonly property real topEdgeA_w: (barPosition === "top") ? 0 : (rightBound - leftBound)
            readonly property real topEdgeB_x: rightBound
            readonly property real topEdgeB_w: 0

            readonly property real bottomEdgeA_x: leftBound
            readonly property real bottomEdgeA_w: (barPosition === "bottom") ? 0 : (rightBound - leftBound)
            readonly property real bottomEdgeB_x: rightBound
            readonly property real bottomEdgeB_w: 0

            readonly property real leftEdgeA_y: topBound
            readonly property real leftEdgeA_h: (barPosition === "left") ? 0 : (bottomBound - topBound)
            readonly property real leftEdgeB_y: bottomBound
            readonly property real leftEdgeB_h: 0

            readonly property real rightEdgeA_y: topBound
            readonly property real rightEdgeA_h: (barPosition === "right") ? 0 : (bottomBound - topBound)
            readonly property real rightEdgeB_y: bottomBound
            readonly property real rightEdgeB_h: 0
            ...
            function applyBarSettings() {
                if (typeof Config === "undefined" || !Config.rawSettings || !Config.rawSettings.bar) return;
            ...
            Connections {
                ...
                function onDataReadyChanged() { floatingWidget.applyBarSettings(); }
                function onRawSettingsChanged() { floatingWidget.applyBarSettings(); }
            }
            ...
            // Only Notepad and DrawAction are ported. Timer / SystemUsage are dropped.
            ...
            Connections {
                ...
                function onActiveIndexChanged() {
                    let idx = FloatingController.activeIndex;
                    if (typeof idx === "number" && idx >= 0 && idx < floatingWidget.tabCount
                            && idx !== floatingWidget.activeIndex) {
                        floatingWidget.activeIndex = idx;
                    }
                }

                function onShowRequested(tab) {
                    if (!floatingWidget.isTargetScreen()) return;
                    let idx = floatingWidget.tabIndexFor(tab);
                    if (idx < 0)
                        idx = (typeof FloatingController.activeIndex === "number") ? FloatingController.activeIndex : 0;
                    floatingWidget.openQuickActions(idx);
                }
            }
            ...
            function tabIndexFor(tab) {
                if (typeof tab !== "string") return -1;
                let t = tab.toLowerCase();
                if (t.indexOf("draw") !== -1) return 1;
                if (t.indexOf("note") !== -1) return 0;
                return -1;
            }

            function setIndex(index) {
                let newIndex = parseInt(index);
                if (isNaN(newIndex) || newIndex < 0 || newIndex >= floatingWidget.tabCount) return;
                floatingWidget.activeIndex = newIndex;
                if (typeof FloatingController !== "undefined" && typeof FloatingController.setIndex === "function")
                    FloatingController.setIndex(newIndex);
            }

            function openQuickActions(index) {
                let newIndex = parseInt(index);
                if (isNaN(newIndex) || newIndex < 0 || newIndex >= floatingWidget.tabCount) newIndex = 0;
                floatingWidget.activeIndex = newIndex;
            ...
            Shortcut { enabled: floatingWidget.isSidebarVisible && !floatingWidget.childIntercepts("Tab"); sequence: "Tab"; onActivated: { floatingWidget.setIndex((floatingWidget.activeIndex + 1) % floatingWidget.tabCount); floatingWidget.kickTimer(); } }
            Shortcut { enabled: floatingWidget.isSidebarVisible && !floatingWidget.childIntercepts("Shift+Tab"); sequence: "Shift+Tab"; onActivated: { floatingWidget.setIndex((floatingWidget.activeIndex + (floatingWidget.tabCount - 1)) % floatingWidget.tabCount); floatingWidget.kickTimer(); } }
            ...
            Shortcut {
                ...
                sequence: "Left"
                onActivated: { floatingWidget.setIndex(Math.max(0, floatingWidget.activeIndex - 1)); floatingWidget.kickTimer(); }
            }
            Shortcut {
                ...
                sequence: "Right"
                onActivated: { floatingWidget.setIndex(Math.min(floatingWidget.tabCount - 1, floatingWidget.activeIndex + 1)); floatingWidget.kickTimer(); }
            }
            Shortcut {
                ...
                onActivated: {
                    ...
                    floatingWidget.setIndex(Math.max(0, Math.min(floatingWidget.tabCount - 1, floatingWidget.activeIndex + step)));
                    ...
                    Item {
                        ...
                        Repeater {
                            ...
                            delegate: Loader {
                                ...
                                // Injected contract consumed by the ported action modules.
                            ...
                            Repeater {
                                ...
                                delegate: Rectangle {
                                    ...
                                    MouseArea {
                                        ...
                                        onClicked: {
                                            if (!isDragging) {
                                                if (!barPill.isActive) floatingWidget.setIndex(index);
