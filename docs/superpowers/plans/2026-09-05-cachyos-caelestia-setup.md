...
**Files:** `data/packages-official.txt`, `data/packages-aur.txt`, `data/flatpaks.txt`, `README.md`, `LICENSE`, `.gitignore`, `docs/supported-system.md`, `tests/bats/manifests.bats`

- [ ] Write `data/packages-official.txt` with one package per line and comments for groups. Include the known official packages: `fish`, `flatpak`, `fwupd`, `hyprland`, `nwg-dock-hyprland`, `pamac-aur`, `snapper`, `btrfs-progs`, `btrfs-assistant`, `bazaar`, `flameshot`, `swappy`, `playerctl`, `pavucontrol`, `cliphist`, `wl-clipboard`, `slurp`, `grim`, `wl-gammarelay-rs`, `ttf-cascadia-code-nerd`, and `ttf-material-symbols-variable`.
- [ ] Write `data/packages-aur.txt` with `caelestia-cli`, `caelestia-shell`, `quickshell-git`, `qt6-m3shapes-git`, `ttf-rubik-vf`, and `zen-browser-bin`, noting that package availability must be checked rather than assumed.
- [ ] Write `data/flatpaks.txt` with the approved optional GUI store/application identifiers only after verifying their exact IDs; do not include a guessed application ID.
...
- [ ] Document supported assumptions: CachyOS/Arch, Hyprland session, Btrfs/Snapper optional, Kitty or another visible terminal, and a logged-in desktop session.
...
**Files:** `install.sh`, `lib/common.sh`, `tests/bats/common.bats`, `tests/fixtures/fake-bin/`
...
- [ ] Add preflight checks for Bash version, interactive/visible terminal guidance, supported distribution, desktop session, and required base commands. Make checks actionable and distinguish hard failures from optional capabilities.
...
- [ ] Install official packages in one planned transaction, refreshing package databases only after confirmation or `--yes`.
...
**Files:** `lib/snapshots.sh`, `lib/configs.sh`, `tests/bats/snapshots.bats`, `docs/troubleshooting.md`
...
- [ ] Implement `--restore TIMESTAMP` with exact backup-directory validation, a confirmation unless `--yes` is supplied, restoration of only managed targets, and a final reload/restart suggestion. Refuse path traversal and symlink escapes.
...
shellcheck lib/snapshots.sh lib/configs.sh
```
...
**Files:** `scripts/render-config.sh`, `lib/configs.sh`, `config/caelestia/**`, `config/fish/conf.d/caelestia-theme.fish`, `config/hypr/**`, `config/nwg-dock-hyprland/**`, `tests/bats/render-config.bats`
...
- [ ] Generate monitor-specific configuration only after querying active outputs; preserve a portable default and honor `--skip-monitor`.
...
**Files:** `lib/integrations.sh`, `config/hypr/config/autostart.lua`, `config/hypr/config/keybinds.lua`, `config/hypr/hyprland.lua`, `config/nwg-dock-hyprland/**`, `tests/bats/integrations.bats`, `docs/troubleshooting.md`
...
shellcheck lib/integrations.sh
```
...
**Files:** `lib/integrations.sh`, `config/caelestia/**`, `config/fish/**`, `docs/supported-system.md`, `tests/bats/desktop-integrations.bats`
...
**Files:** `scripts/cachy-health`, `lib/health.sh`, `tests/bats/health.bats`, `docs/supported-system.md`
...
shellcheck scripts/cachy-health lib/health.sh
```
...
**Files:** `install.sh`, `lib/*.sh`, `tests/bats/installer.bats`, `tests/fixtures/`
...
**Files:** `.github/workflows/ci.yml`, `scripts/check-source-policy.sh`, `.shellcheckrc`, `.editorconfig`, `README.md`, `docs/troubleshooting.md`, `tests/bats/**`
...
**Files:** `docs/validation/live-host-YYYY-MM-DD.md`, `README.md`
